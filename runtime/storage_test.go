				...
				Source: []byte(`
                    ...
                    transaction {
                        prepare(signer: AuthAccount) {
                            ...
                            signer.link<&String>(/private/a, target:/storage/first)
                            signer.link<&{Foo.Collection}>(/private/b, target:/storage/second)

                            signer.link<&String>(/public/a, target:/storage/first)
                            signer.link<&{Foo.Collection}>(/public/b, target:/storage/second)
                        }
                    }
                `),
			},
                    ...
                    transaction {
                        prepare(account: AuthAccount) {
                            ...
                            account.forEachPublic(fun (path: PublicPath, type: Type): Bool {
                                var cap = account.getCapability<&{Foo.Collection}>(path)
                                cap.check()
                                total = total + 1
                                return true
                            })
